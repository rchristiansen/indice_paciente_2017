<?php
	...
	function listarPaciente($objCon, $parametros){
		$sql="SELECT
				...
				comuna.comuna AS COM_Descripcion,
				ciudad.CIU_Descripcion,
				prevision.prevision AS PREV_Descripcion,
				convenioinstitucion.convinstNombre

			FROM
			paciente
			LEFT JOIN doc_extranjero ON doc_extranjero.id_doc_extranjero = paciente.id_doc_extranjero
			LEFT JOIN region ON region.REG_Id = paciente.region
			LEFT JOIN comuna ON comuna.id = paciente.idcomuna
			LEFT JOIN ciudad ON ciudad.CIU_Id = paciente.ciudad
			LEFT JOIN prevision ON prevision.id = paciente.prevision
			LEFT JOIN convenioinstitucion ON convenioinstitucion.convinstInstCod = paciente.conveniopago";
		...
		function listarPacienteExterno($objCon, $parametros){
			...
			paciente
			LEFT JOIN doc_extranjero ON doc_extranjero.id_doc_extranjero = paciente.id_doc_extranjero
			LEFT JOIN region ON paciente.region = region.REG_Id
			LEFT JOIN comuna ON comuna.id = paciente.idcomuna
			LEFT JOIN ciudad ON ciudad.CIU_Id = paciente.ciudad
			LEFT JOIN prevision ON prevision.id = paciente.prevision
			LEFT JOIN convenioinstitucion ON convenioinstitucion.convinstInstCod = paciente.conveniopago";
		...
		function grabar_paciente($objCon, $parametros){
			...
			$sql="INSERT INTO paciente( extranjero,
								...
								VALUES('{$parametros['frm_extranjero']}',
									   ...
									   '{$parametros['frm_AP']}',
									   '{$parametros['frm_AM']}',
									    CONCAT('{$parametros['frm_nombreCalleIndicePaciente']}', ' ', '{$parametros['frm_numeroDireccionIndicePaciente']}', ' ', '{$parametros['frm_direccion']}'),
		...
		function actualiza_paciente($objCon, $parametros){
			$sql="UPDATE paciente
				  SET    rut                        = '{$parametros['frm_rut_pac']}',
						 ...
						 dv                         = '{$parametros['dv']}',
						 direccion                  = CONCAT('{$parametros['frm_nombreCalle']}', ' ', '{$parametros['frm_numeroDireccion']}', ' ', '{$parametros['frm_direccion']}')
...
*/ 				  WHERE paciente.id ='{$parametros['frm_id_paciente']}'";
		...
		$sql="UPDATE paciente";
			...
			$condicion .= ", paciente.direccion =  CONCAT('{$parametros['frm_nombreCalle']}', ' ', '{$parametros['frm_numeroDireccion']}', ' ', '{$parametros['frm_direccion']}')";
