// var raiz = "../../../../";
var raiz_ip = "http://10.6.21.22/indice_paciente_2017";
...
function ajaxContent(url,parametros,contenedor, mensaje, sincrono, callback){
	...
	$.blockUI({
		message: '<img height="80" src="'+raiz_ip+'/assets/img/loading-5.gif"/><label class="loadingBlock">'+msg+'</label>',
		css: {
			border: 'none',
	...
	*/
	...
	if(typeof (callBack) === "function"){
		...
		if(typeof mensaje !== "undefined"){
			$.blockUI({
				message: '<img height="80" src="'+raiz_ip+'/assets/img/loading-5.gif"/><label class="loadingBlock">'+mensaje+'</label>',
				baseZ: 2000,
				css: {
					border: 'none',
					backgroundColor:'transparent'
	...
	var modal ='<div id="'+id+'" class="modal fade" tabindex="-1" role="dialog" >'+
				  		'<div class="modal-dialog" role="document" style="width: '+ancho+'; height:'+alto+';">'+
				    		'<div class="modal-content"  id="modalContenido" >'+
						      ...
						      '</div>'+
						      '<div class="modal-footer">'+
						      '<button type="button" class="btn btn-default" data-dismiss="modal" id="cancelar'+time+'"> Cerrar</button>';
						      for (var btn in botones){
							     modal+='<button id="'+botones[btn].id+'" type="button" class="btnModalesFrm-'+id+" "+botones[btn].class+'">'+botones[btn].value+'</button>';
							   }
								modal+='</div>' +
...
function tabla(referencia){
	var tabla = $(referencia).DataTable({
						...
						// "processing": true,
      					// "serverSide": true,
      					// "ajax": "'+raiz_ip+'/views/modules/Pacientes/busquedaPaciente.php",
						language: {
...
function ajaxForm(url,formulario, callBack){
	...
	$.ajax({
		...
		beforeSend: function() {
	 		if(typeof mensaje !== "undefined") {
	 			$.blockUI({
	 				message: '<img height="70" src="'+raiz_ip+'/assets/img/loading-5.gif"/><label class="loadingBlock">'+mensaje+'</label>',
	 				css: {
	 					border: 'none',
	 					backgroundColor:'transparent'
	 				}
