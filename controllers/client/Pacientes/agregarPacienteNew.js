				...
				// alert("4")					
					var pacienteFonasaBuscar = function(response){	//FUNCION 3						
						switch(response.status){
							case "success":														
								$('#pacienteFonasa').modal( 'hide' ).data( 'bs.modal', null ); // para eliminar modal una vez ejecutada								
								modalFormulario('Datos Fonasa','/indice_paciente_2017/views/modules/Fonasa/fonasaAgregarPaciente.php','datosFonasa='+response.datosFonasa+'&banderaGES='+$("#banderaGES").val(),'#pacienteFonasaCertificado','80%','auto','');							
							break;
						...
						var  grabarPaciente = function(){
							...
							var grabar = function(response){
								switch(response.status){								
									...
									if(response.afrodescendiente_select && response.afrodescendiente){	
										$('#'+response.afrodescendiente_select).val(response.afrodescendiente);													
									}

									if(response.prais_select && response.prais){	
										$('#'+response.prais_select).val(response.prais);													
									}

									if(response.paisNacimiento_select && response.paisNacimiento){	
										$('#'+response.paisNacimiento_select).val(response.paisNacimiento);													
									}

									if(response.region_select && response.region){	
										$('#'+response.region_select).val(response.region);													
									}

									if(response.ciudad_select && response.ciudad){	
										$('#'+response.ciudad_select).val(response.ciudad);													
									}

									if(response.comuna_select && response.comuna){	
										$('#'+response.comuna_select).val(response.comuna);													
									}

									if(response.sistemaExterno == 'DAU'){
										if(response.region_select && response.region){	
											$('#'+response.region_select).val(response.region);													
										}
				
										if(response.ciudad_select && response.ciudad){	
											obtenerValorCiudad(response.ciudad);	
										}
										
										if(response.comuna_select && response.comuna){	
											obtenerValorComuna(response.comuna);	
										}
									}

									if(response.calle_input && response.calle){	
										$('#'+response.calle_input).val(response.calle);													
									}

									if(response.numero_input && response.numero){	
										$('#'+response.numero_input).val(response.numero);													
									}

									if(response.sector_select && response.sector){	
										$('#'+response.sector_select).val(response.sector);													
									}

									if(response.otrosTelefonos_input && response.otrosTelefonos){	
										$('#'+response.otrosTelefonos_input).val(response.otrosTelefonos);													
									}
