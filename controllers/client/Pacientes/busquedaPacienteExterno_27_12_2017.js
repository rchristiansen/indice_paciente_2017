$(document).ready(function(){
	...
	$("#btnNuevoPaciente").click(function(){
		...
		var fonasa 		  = $("#fonasa").val();
		// var doc_documento = $("#doc_documento").val();		
		ajaxContent('/indice_paciente_2017/views/modules/Pacientes/agregarPaciente.php','sistemaExterno='+sistemaExterno+'&nombres=frm_nombres_dau&run=frm_rut&AP=frm_AP_dau&AM=frm_AM_dau&fechaNac=frm_Naciemito&calcularEdad=labelEdad&sexo=frm_sexo&etnia=frm_etnia&cap=frm_centroAtencion&nac=frm_Nacionalidad&direccion=frm_direccion&correo=frm_correo&telefonoCelular=frm_telefonoCelular&telefonoCelularFijo=frm_telefonoFijo&prevision=frm_prevision&prevision=frm_prevision&formaPago=frm_formaPago&idPaciente=idPacienteDau&pacienteFall=pacienteFallDau&fonasa='+fonasa+'&tipoDocumentoLabel=tipoDocumentoDau&doc_documento=id_doc_documentoDau','#contenidoPaciente','', true); 
	});
		...
		if(response.datosPacientes.datos[0]['extranjero']=="S"){
	 				...
	 				switch(response.status){
	 					case "success":	 	
	 						...
	 						if(response.datosPacientes.datos[0]['extranjero']=="S" && response.datosPacientes.datos[0]['rut']!="" && response.datosPacientes.datos[0]['rut_extranjero']!=""){
	 							// alert("especial")
	 							if(tipoDocumentoLabel){
	 								document.getElementById(tipoDocumentoLabel).innerHTML = "RUN"; //label
	 							}
								...
								if(sexo){
									if(response.datosPacientes.datos[0]['sexo']!=""){
										var sexoPaciente = response.datosPacientes.datos[0]['sexo'];
									}else{
										var sexoPaciente = 'D';
									}									
							...
							switch(response.fonasa){
								...
								var funcion = function(response){
									switch(response.status){
										...
										break;
										default:    modalMensaje2("Error de Conexión Fonasa", 'El servicio de <b>Fonasa no responde</b>, intentelo de nuevo mas tarde.', '', 550, 300, 'danger', 'remove');
									}
								}
